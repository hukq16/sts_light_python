...
# ORIGINAL LINE: CardInstance(CardId id, bool upgraded =false) : id(id), upgraded(upgraded)
        def __init__(self, id, upgraded = False):
            self._initialize_instance_fields()

            self.id = sts.CardId(id)
            self.upgraded = upgraded
            self.cost = getEnergyCost(id, upgraded)
            self.costForTurn = self.cost

# C++ TO PYTHON CONVERTER TASK: There is no Python equivalent to multiple constructors:
# ORIGINAL LINE: CardInstance(const Card &card) : CardInstance(card.getId(), card.isUpgraded())
        def __init__(self, card):
            self._initialize_instance_fields()

            self(card.getId(), card.isUpgraded())
            if self.id == CardId.SEARING_BLOW:
                self.specialData = short(card.getUpgraded())
            else:
                self.specialData = card.misc

        # C++ TO PYTHON CONVERTER TASK: C++ attributes are not converted to Python:
        # ORIGINAL LINE: [[nodiscard]] CardId getId() const
# C++ TO PYTHON CONVERTER WARNING: 'const' methods are not available in Python:
# ORIGINAL LINE: CardId getId() const
        def getId(self):
            return self.id

        # C++ TO PYTHON CONVERTER TASK: C++ attributes are not converted to Python:
        # ORIGINAL LINE: [[nodiscard]] CardType getType() const
# C++ TO PYTHON CONVERTER WARNING: 'const' methods are not available in Python:
# ORIGINAL LINE: CardType getType() const
        def getType(self):
...
# ORIGINAL LINE: bool canUpgrade() const
        def canUpgrade(self):
            return ((not self.upgraded) or self.id == CardId.SEARING_BLOW) and self.getType() != CardType.CURSE and self.getType() != CardType.STATUS
...
# ORIGINAL LINE: bool usesSpecialData() const
        def usesSpecialData(self):
            if (self.getId() == CardId.SEARING_BLOW) or (self.getId() == CardId.RAMPAGE) or (self.getId() == CardId.GENETIC_ALGORITHM) or (self.getId() == CardId.RITUAL_DAGGER):
                return True
...
# ORIGINAL LINE: bool canUse(const BattleContext &bc, int target, const bool inAutoplay) const
        def canUse(self, bc, target, inAutoplay):
            if self.requiresTarget() and (bc.monsters.areMonstersBasicallyDead() or (not bc.monsters.arr[target].isTargetable())):
                return False
            ...
            if self.getType() == CardType.ATTACK:
                ...
                if self.id == CardId.SECRET_TECHNIQUE:
                    const bool haveSkill = std::find_if(bc.cards.drawPile.begin(), bc.cards.drawPile.end(), lambda c : c.getType() == CardType.SKILL) is not bc.cards.drawPile.end(); # todo maybe speed up
                    if not haveSkill:
                        return False
                elif self.id == CardId.SECRET_WEAPON:
                    const bool haveAttack = std::find_if(bc.cards.drawPile.begin(), bc.cards.drawPile.end(), lambda c : c.getType() == CardType.ATTACK) is not bc.cards.drawPile.end(); # todo maybe speed up
                    if not haveAttack:
                        return False
    ...
    def left_shift(self, os, c):
        os << "(" << c.getName() << ("+" if c.upgraded else "") << "," << str(c.uniqueId) << "," << int(c.cost) << "," << int(c.costForTurn)
